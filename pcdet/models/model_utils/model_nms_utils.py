...
def class_agnostic_nms(box_scores, box_preds, nms_config, score_thresh=None):
    ...
    if box_scores.shape[0] > 0:
        # if box_scores.dim() == 2:
        #     box_scores_flat = box_scores.reshape(-1)
        #     box_scores_nms_flat, indices = torch.topk(box_scores_flat, k=min(nms_config.NMS_PRE_MAXSIZE, box_scores.shape[0]))
        #     box_scores_nms = box_scores_nms_flat.reshape((-1, box_scores.shape[1]))
        # else:
