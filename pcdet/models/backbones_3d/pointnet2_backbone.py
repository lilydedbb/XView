...
class PointNet2MSG(nn.Module):
    ...
    def forward(self, batch_dict, inkeys=None, outkeys=None):
        ...
        if outkeys is None:
            ...
            batch_dict[outkeys[0]] = point_features.view(-1, point_features.shape[-1])
            batch_dict[outkeys[1]] = torch.cat((batch_idx[:, None].float(), l_xyz[0].view(-1, 3)), dim=1)

        return batch_dict


class MPointNet2MSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        super().__init__()
        self.pointnet2_list = []
        for i in range(len(kwargs['point_cloud_ranges'])):
            kwargs['pview'] = True
            pointnet2 = PointNet2MSG(model_cfg, input_channels[i], **kwargs)
            self.pointnet2_list.append(pointnet2)
            self.add_module(f'pointnet2_{i}', pointnet2)

    def forward(self, batch_dict):
        num_views = len([key for key in batch_dict if key.startswith('points_pviews_')])
        for i in range(num_views):
            inkeys = f'points_pviews_{i}'
            outkeys = [f'pview_point_features_{i}', f'pview_point_coords_{i}']
            batch_dict = self.pointnet2_list[i](batch_dict, inkeys=inkeys, outkeys=outkeys)
        return batch_dict
