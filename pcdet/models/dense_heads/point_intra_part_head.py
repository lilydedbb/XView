...
class PointIntraPartOffsetHead(PointHeadTemplate):
    ...
    def __init__(self, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        self.spherical_point_cloud_ranges = kwargs['spherical_point_cloud_ranges'] \
            if 'spherical_point_cloud_ranges' in kwargs else None
        self.spherical_voxel_sizes = kwargs['spherical_voxel_sizes'] \
            if 'spherical_voxel_sizes' in kwargs else None
    ...
    def cart_to_spherical(self, coords, spherical_point_cloud_range, spherical_voxel_size):
        ...
        brpt_ = torch.stack((coords[:, 0], r, psi, theta), dim=-1).long()
        return brpt_
    ...
    def forward(self, batch_dict):
        ...
        if getattr(self.model_cfg, 'MULTI_VIEWS', None):
            point_coords = batch_dict['point_coords']
            num_views = len([key for key in batch_dict.keys() if key.startswith('spherical_point_features_')])
            for i in range(num_views):
                dense_spherical_points_features = batch_dict[f'spherical_point_features_{i}'].dense()
                brpt_ = self.cart_to_spherical(point_coords.float(), self.spherical_point_cloud_ranges[i], self.spherical_voxel_sizes[i])
                if self.model_cfg.FUSE_METHOD == 'add':
                    point_features = point_features + \
                        dense_spherical_points_features[brpt_[:, 0], :, brpt_[:, 3], brpt_[:, 2], brpt_[:, 1]]
                else:
